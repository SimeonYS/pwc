...
class PwcPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('pwc.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `pwc`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM pwc WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO pwc (date, title, link, content)"
